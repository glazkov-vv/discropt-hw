...
int main() {
    ...
    while (std::getline(in, line)) {
        ...
        auto result = Solve(testCase.MaxWeight, testCase.Objects);
        try {
            Check(testCase.MaxWeight, testCase.Objects, result);
        } catch (const char* ex) {
        ...
        i64 score = 0;
        for (i64 id : result) {
            score += testCase.Objects[id].Cost;
        }
